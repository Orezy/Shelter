...
class SelectedPetsControllerTest < ActionController::TestCase
  ...
  test "should create selected_pet" do
    assert_difference('SelectedPet.count') do
      post :create, pet_id: pets(:one).id #, selected_pet: { cart_id: @selected_pet.cart_id, pet_id: @selected_pet.pet_id }
    end

    assert_redirected_to shelter_index_path#cart_path(assigns(:selected_pet).cart) #selected_pet_path(assigns(:selected_pet))
  end
