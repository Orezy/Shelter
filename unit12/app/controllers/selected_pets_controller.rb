class SelectedPetsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @selected_pet.save
        format.html { redirect_to shelter_index_path, notice: 'Pet was selected.' }
        format.js {}
        format.json { render :show, status: :created, location: @selected_pet.cart }
      else
  ...
  def destroy
    # Pet.find(@selected_pet.pet_id).set_available
    @selected_pet.destroy
    @pets = Pet.get_available_pets
    # @cart = set_cart
    respond_to do |format|
      format.html { redirect_to shelter_index_path, notice: 'Pet was successfully removed from the cart.' }
      format.js {}
